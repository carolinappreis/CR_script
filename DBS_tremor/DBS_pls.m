...
load(strcat('C:\Users\creis\OneDrive - Nexus365\Phasic_DBS\patient data\raw_data\P0',num2str(iii(numb)),'\P0',num2str(iii(numb)),'_PLS.mat'))
...
% plot(time2,tremor2)
% hold on
% plot(time2(1,start),tremor2(1,start),'ko')
% plot(time2(1,ending),tremor2(1,ending),'ro')
...
% plot(time,data(2,:),'LineWidth',1,'Color',[0.5 0.5 0.5])
% hold on
% plot(time,data(1,:),'LineWidth',1)
% plot(time,dc*2,'LineWidth',1)
...
run(strcat('pt_0',num2str(iii(numb)),'_spec.m'))
...
for rr=1:length(start)
    ...
    trace(rr,:)=envelope(start(rr)-1000:start(rr)+90000-1); %%% clean short spiral and remove 5s cut
    idx_t(rr,:)=[bef_s(rr) ending(rr)-5000]; %%% clean short spiral and remove 5s cut
    change(1,rr)=(mean(envelope(ending(rr)-5000:ending(rr)))-mean(envelope(start(rr)-5000:start(rr))))./mean(envelope(start(rr)-5000:start(rr)));
end
