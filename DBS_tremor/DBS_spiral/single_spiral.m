...
for iii =  1
    ...
    for i = 1:length(out.b_trials{iii,co})
        ...
        gi{i,1}= [out.b_trials{iii,co}(i):out.e_trials{iii,co}(i)];
    ...
    for lh=1:size(out.h_up{1,1},1)
        ...
        for o=1:3
            [v_new,out]= split_vector_length(x_signal(o,out.h_up{iii,co}{lh,1}), L,out,iii,co,o);
...
%         
        ...
        %     x_all{1,1}=[pc_t{1,1}; pc_t{1,2};pc_t{1,3}];
        x_all{1,1}=[pc_t{1,1}];
        runs{1,1}=1:length(x_all{1,1});
        
        %     runs{1,1}=1:length(pc_t{1,1});
        %     runs{1,2}=length(pc_t{1,1})+1:length(pc_t{1,2})+length(pc_t{1,1});
        %     runs{1,3}=length(pc_t{1,2})+length(pc_t{1,1})+1:length(x_all{1,1});
        k=3;
        ...
        %     figure()
        %     scatter3([pc_t{iii,1}(:, 1); pc_t{iii,2}(:, 1); pc_t{iii,3}(:, 1)], [pc_t{iii,1}(:, 2); pc_t{iii,2}(:, 2); pc_t{iii,3}(:, 2)], [pc_t{iii,1}(:, 3); pc_t{iii,2}(:, 3); pc_t{iii,3}(:, 3)], 10, c)
...
%         
        ...
        %     title('NS2')
        %     subplot(4,1,4)
        %     [l h]=silhouette(pc_t{iii,3},c(runs{1,3}));
        %     title('NS3')
        i=1;
