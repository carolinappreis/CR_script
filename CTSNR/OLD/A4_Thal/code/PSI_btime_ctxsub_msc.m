...
for t=1:size(freq,1)
    ...
    for r=1:size(data,1); %electrodes
        ...
        coh_thal=[];
        filt_thal=[];
        [power1,f]=pwelch(data{r,1}(1,:),1000,[],1000,1000); %ctx power spectra
        %         plot(f,power1)
        %         xlim ([0 100])
        %         ylim ([0 200000])
        for rr=2:size(data{r,1},1) %contacts
            ...
            filt_ctx=filtfilt(b,a,data{r,1}(1,:)); %filt ctx signal
            ...
            if   sum(Pxx_ind(freq(t)-5:freq(t)+5))./sum(Pxx_ind(1:end))>0.1; %sum(power(freq(t)-5:freq(t)+5))./sum(power(1:end))>0.1 
                ...
                non_norm=squeeze(angle(hilbert(filt_ctx))-angle(hilbert(filtfilt(b,a,data{r,1}(rr,:))))); %dif angles between ctx-subctx 
