        ...
        for ra=2:size(WaveA{subj(r),1},1);
            for rb=2:size(WaveB{subj(r),1},1);
                ...
                if sum(Pxx_indA(freq))./sum(Pxx_indA(1:end))>0.1 && sum(Pxx_indB(freq))./sum(Pxx_indB(1:end))>0.1
                    ctcts=ctcts+1;
                    %                     non_norm=angle(hilbert(filtfilt(b,a,WaveA{subj(r),1}(ra,:))))-angle(hilbert(filtfilt(b,a,WaveA{subj(r),1}(1,:)))); %dif angles between ctx-subctx
                    %                                          non_norm=angle(hilbert(filtfilt(b,a,WaveB{subj(r),1}(rb,:))))-angle(hilbert(filtfilt(b,a,WaveB{subj(r),1}(1,:))));
...
%                     channel_bursts=WaveB{subj(r),1}(rb,:);
                    %                    channel_bursts=WaveA{subj(r),1}(ra,:);
                                          channel_bursts=WaveA{subj(r),1}(1,:);
    ...
    if size(clust_b,1)>1
        ...
        title ('PSI BZ-Rt in ctx beta bursts')
        xlim ([0 400])
        xticks([0:100:400])
...
%         cd ('/Users/Carolina/Documents/GitHub/CRcode/codes_thal/A4_Thal/code')
        ...
        if ~isempty(beg)
            patch([beg(1) beg(end) beg(end) beg(1)],[0 0 1 1],color_b,'FaceAlpha',[0.2],'EdgeColor','none')
        end
    end
...
% savefig('clust_snrb_snr_bz')
% saveas(gca,'clust_snrb_snr_bz.png')
