...
function [fig]=broad_cycy(data,ecog,name)
...
for j=1:size(data,1)
    ...
    clearvars -except name ecog data srn j b a check vec_lg pref_pha tt idx gg all allall
    ctx=ecog(j,:);
    ...
    if (circ_rtest(ang))<0.05
        ...
        % %         arr=reshape(block,1,size(block,1)*size(block,2));
        % %         non_resamp=floor(numel(unique(arr))./length(arr)*100)
        % %         B = unique(arr);
        % %         Ncount = histc(arr, B);
        % %         histogram(Ncount)
        % %         box('off')
        % %         ylabel ('counts')
        % %         xlabel('#times resampled')
...
for i=1:length(cy_plots)
    ...
    if i==1
        polarplot([0 ang_m(1,cy_plots(i))], [0, vec_m(1,cy_plots(i))],'Color',cl,'linewidth',1,'Marker','d')
    else
...
hold on
xline(6,'--',{'burst onset'},'LabelOrientation','horizontal','LabelVerticalAlignment','bottom','Color','r','LineWidth',3)
xlim([0 11])
ylim([0 0.6])
yticks(0:0.2:0.6)
...
errorbar(x,y,err,'-d','LineWidth',1,5,'Color',color_b{1,2})
hold on
% xline(11,'--',{'burst onset'},'LabelOrientation','horizontal','LabelVerticalAlignment','bottom','Color',[0.5 0.5 0.5],'LineWidth',3)
xlim([0 22])
ylim([0 0.6])
yticks(0:0.2:0.6)
xlabel('Number of {\beta} cycles')
