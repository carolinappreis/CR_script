    ...
    for ct=1:size(thal,1)
        ...
        for o=1:size(onset1,1)
           onset=onset1{o,1};
            ...
            ref_amp(pr,o,ct,:)=nanmedian(ref_seg,1);
            
            clear al_seg surr_seg onset ref_seg
        end
...
clearvars -except burst_amp surr_amp el name ref_amp

data=cell(1,3);
cond={'burst_amp' 'surr_amp' 'ref_amp'};
...
if name=='bz'
    color_ref={squash blood};
else
    color_ref={sky aegean};
end
color_b={[0.5 0.5 0.5]  [0 0 0]};
...
fig=figure;
for on=1:2
    clear A; A=squeeze(data{co,3}(on,:,:));
    y2=100.*(mean(A));
    y1=100.*(mean(A)+std(A)./sqrt(size(A,1)));
    y3=100.*(mean(A)-std(A)./sqrt(size(A,1)));
    %         y1= 100.*(mean(A)+std(A));
