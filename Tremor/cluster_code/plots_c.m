...
%            datas(i,:)=eval(['nanmedian(out.arc' num2str(i) '{' num2str(iii) ',2}{m_ax,1})']);
        %%% OR normalised to the median of nstim dist
           ...
           datas_old(i,:)=eval(['nanmedian(out.arc' num2str(i) '{' num2str(iii) ',2}{m_ax,1})']); 
           
 %%% check normality of distribution with changes in instimualted data
% %            figure(5+i)
% %          
% %                if iii~=3
% %                    subplot(2,5,iii)
...
% %            
        ...
        if ~isempty(find(datas_old(i,:) > prctile(nsdatas(i,:), 99.7917) | datas_old(i,:)< prctile(nsdatas(i,:), 0.2083)))
            pha{iii,i}=find(datas_old(i,:) > prctile(nsdatas(i,:), 99.7917) | datas_old(i,:)< prctile(nsdatas(i,:), 0.2083)); %% find phases with significant changes in tremor severity
        else
            pha{iii,i}=NaN;
        end
