...
Ecog=BZ.ctx(BZ.alpha_rats,:);
for i =1:size(BZ.alpha_rats,1)
    for ii=1:size(BZ.alpha_idx{BZ.alpha_rats(i),1},2)
        hp=BZ.alpha_idx{BZ.alpha_rats(i),1}(1,ii);
        data_all{i,1}(ii,:)=BZ.sua{BZ.alpha_rats(i),1}(hp,:);
        clear hp
    end
end

% subj= BZ.alpha_rats(ismember(BZ.alpha_rats,BZ.alpha_rats));
...
for j=1:size(data_all,1)
    ...
    data=data_all{j,1};
    ecog=Ecog(j,:);
    clearvars -except j ii srn Ecog data ecog data_all  rec_pa1 rec_npa1 region_pl region_spl region_npl region_nspl
