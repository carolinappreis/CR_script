...
clear all
ad=struct; ad.metrics=cell(1,1); ad.spirals=cell(1,1); ad.sig=cell(1,1); ad.signames={'data1';'data2';'time';'envelope'};ad.metricnames={'subj';'cond';'trials';'nspirals';'nquadrants';'mean';'peak2peak';'variance'};
...
for iii=3
    ...
    for co=3
        ...
        for trial=1:length(ntrial)
            ...
            %
            %             dx = [signal1(1,:); signal1(2,:)];
            %             [pc, score, latent, tsquare, explained] = pca(dx');
            %             signal1(3,:)= signal(1,:).*pc(1,1)+ signal(2,:).*pc(2,1);
