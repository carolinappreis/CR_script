...
for r=1:size(data,1); %electrodes
    ...
    for rr=2:size(data{r,1},1) %contacts
        ...
        if  sum(Pxx_ind(fq))./sum(Pxx_ind(1:end))>0.1
            ...
            load('SNR.mat');id_rat=([1 3 11 12])';
            onset1= SNR.ctxb_all{1, 1};
...
for cb=1:size(clust_b,1)
    ...
    %     color_b=color_snr; color_s=[0 0 0];
...
clearvars -except animals stat_idx clust_b clust_s clust_s power_coh power_ctx ctx_b1 psi_b  psi_bsem psi_s psi_ssem ctx_b ctx_sb time
