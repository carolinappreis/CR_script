...
for ep=1:size(t,2);
    for th=1:size(tremor3,1)
        ...
        for i=1:length(t(ep))
            for tr=1:length(s)
%                 RS_raw1=[RS_raw1 tremor3(th,s(tr):(s(tr)+t(ep)-1))];
%                 RS_t1=[RS_t1 filt_t3(th,s(tr):(s(tr)+t(ep)-1))];
%                 RS_e1=[RS_e1 env_t3(th,s(tr):(s(tr)+t(ep)-1))];
%                 RS_dc1=[RS_dc1 dc_t3(th,s(tr):(s(tr)+t(ep)-1))];
