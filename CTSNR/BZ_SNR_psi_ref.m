...
for iii=1:size(combo,1)
    ...
    for ii=2:3
        ...
        [data,baseline]=change_psi_with_ref(dat1,dat2);
        el=500;
        time=1:el+1;
        for yy=1:2
            fig=figure;
            for on=1
                %                         on=1:2
                clear A; A=squeeze(data{1,yy}(on,:,:));
                if size(A,1)>1
                    val=mean(A);
                else
                    val=A;
                ...
                cl=(col{co2,1}(on,:)+col{co2,1}(on,:))./2;
            ...
            xlabel('Time (ms)');
            title([eval(sprintf('region{1,co%d}',1)) eval(sprintf('region{1,co%d}',2))])
            
            str = ['psi:',num2str(baseline(yy))];
            t = text(-1.5,-2.5,str);
            s = t.FontSize;
            t.FontSize = 12;
        end
