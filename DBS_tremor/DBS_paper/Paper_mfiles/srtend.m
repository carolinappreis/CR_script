function [peak_ax, start, ending, yy, h_up,ts,te]= srtend(d,samplerateold,samplerate,iii,co,start,ending,yy,h_up,spiral)
...
%         
        ...
        if spiral==0
            ...
            bins=1:2:length(sp);
            ts{iii,co}=floor((sp(bins)./samplerateold)*samplerate)+addon;
            te{iii,co}=floor((ep(bins)./samplerateold)*samplerate)+addon;
