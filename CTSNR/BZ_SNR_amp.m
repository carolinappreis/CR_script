...
%     1:size(region,2)
    ...
    for co2=1:size(region,2)
        clearvars -except region Ecog BZ SNr col co1 co2 dat1 new_data
        dat2=eval(region{1,co2});
        if co1~=co2
        [data]=change_amp(dat1,dat2);
%         new_data{1,co2-1}=new_d;
            el=500;
            time=1:el+1;
