...
for pr=1:size(bua,1)
    ...
    for ct=2:size(bua{pr,1},1)
        ...
        if (sum(Pxx_ind(16:36))/sum(Pxx_ind(1:end)))>0.1
            coh_r{pr,1}(1,1:length(bua{pr,1}(1,:))) = bua{pr,1}(1,:);
            
            coh_r{pr,1}(m,1:length(bua{pr,1}(1,:))) = bua{pr,1}(ct,:);
            m=m+1;
        end
        clear Pxx_ind_beta
    end
end
raw_coh=coh_r(~cellfun('isempty',coh_r));
...
%     y3=y2-(std(data));
%             if ii~=3
%                 y1=log10(y1);y2=log10(y2);y3=log10(y3);
%             end
