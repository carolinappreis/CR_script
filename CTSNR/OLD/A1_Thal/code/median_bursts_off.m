clearvars -except stats_bursts time subjects muafilt A nfile_i state_mat nn x med_thal_cell med_ctx chanofinterest B WaveData WaveData_DC ind_p
...
for i=2:(length(begin3)-1)
        %if (ending3(i)-begin3(i))<=median_b && (begin3(i+1)-ending3(i))>=150 && (begin3(i)-ending3(i-1))>=150
    if (ending3(i)-begin3(i))>=median_b && (begin3(i+1)-ending3(i))>=150 && (begin3(i)-ending3(i-1))>=150
        ind_b1=[ind_b1 i];
    end
end
...
offset=offset(start:stop);
    
    for j=1:length(offset)
            output_ctx(j,1:1001)=(env(offset(j)-500:offset(j)+500)-median(env(offset(j)-500:offset(j))))./median(env(offset(j)-500:offset(j)));
    end
...
for z=1:(thal-1) 
    for j=1:length(offset)
        if offset(j)-500>0 && offset(j)+500<length(thal_env)
            ...
            output(z,j,1:1001)= ((thal_env(z,offset(j)-500:offset(j)+500)-median(thal_env(z,offset(j)-500:offset(j))))./median(thal_env(z,offset(j)-500:offset(j))));
        end
    end
end
...
%check4 - (size(muafilt,1)-1).*length(offset)==size(output1,1)
...
m=1;
median_probes=zeros(size(offset,2),1001);
